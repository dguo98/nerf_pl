...
CUDA_VISIBLE_DEVICES=${GPU} python train.py --dataset_name ${DATASET} --img_downscale ${IMG_DS} --use_cache --encode_a --encode_t --N_importance 64 --N_samples 64 --beta_min 0.03 --N_vocab 1500 --num_epochs ${EPOCHS} --batch_size ${BS} --optimizer adam --lr ${LR} --lr_scheduler cosine --exp_name ${EXP_NAME} --root_dir ${DATA_DIR}  --model ${MODEL} --lambda_u ${LAMBDA_U} --n_features ${N_FEAT} --backbone_res ${BACKBONE_RES} --box_warp ${BOX_WARP} --use_xyz_net ${USE_XYZ_NET} --n_xyz_dim ${N_XYZ_DIM} --mode ${MODE} 1> ${LOG_DIR}/log.out 2> ${LOG_DIR}/log.err
