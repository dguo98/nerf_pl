...
def generate(gpu, epoch, bs, lr, img_ds, model, box_warp, n_feat, back_res, use_xyz_net, n_xyz_dim,mode,temp, lambda_u):
    global exp_id
    exp_id += 1
...
if __name__ == "__main__":
    ...
    epoch=20  # sufficient for benchmark
    bs=1024
    lr=5e-4
    img_ds=8
    model="NeRFTriplane"
        ...
        for lr in [5e-4]:
            for box_warp in [2.0]:
                for model in ["NeRF"]:
                    ...
                    gpu=(ind%5)
                    ind+=1
                    script = generate(gpu, epoch, bs, lr, img_ds, model, box_warp, n_feat, back_res, use_xyz_net, n_xyz_dim, mode, temp,lambda_u)
    ...
    for temp in ["template.sh"]:
        for lr in [5e-4]:
            for box_warp in [2.0]:
                for model in ["NeRFTriplane"]:
                    for mode in ["debug5", "debug6"]:
                        if model == "NeRFTriplane":
                            back_res=512
                            mode="debug5"
                        else:
